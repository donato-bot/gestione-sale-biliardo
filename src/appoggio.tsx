...
export default function DashboardSala() {
  ...
  const salvaNuovoTorneo = async (staffId: string) => {
    ...
    await supabase.from('tornei').insert([{ 
      sala_id: currentSalaId, nome: newTorneoNome, data_inizio: newTorneoData, 
      quota_iscrizione: parseFloat(newTorneoQuota) || 0, stato: 'iscrizioni', iscritti: [], tabellone: []
    }]);
    alert("✅ Torneo Creato!");
  ...
  const salvaModificaProdotto = async (staffId: string) => {
    await supabase.from('prodotti').update({
      nome: editProdName, prezzo_vendita: parseFloat(editProdPrice), quantita_stock: parseInt(editProdStock) || 0
    }).eq('id', editProdId);
    await refreshDati(currentSalaId!);
  ...
  const salvaRicarica = async (staffId: string) => {
    ...
    await supabase.from('movimenti_cassa').insert([{
      sala_id: currentSalaId, tipo: 'entrata', categoria: 'ricarica_vip',
      metodo_pagamento: rechargeMetodo, importo: importoVal.toFixed(2),
      descrizione: `Ricarica Tessera: ${socioToRecharge.cognome} ${socioToRecharge.nome}`, staff_id: staffId
    }]);
  ...
  const confermaChiusura = async (metodo: any, staffId: string) => {
    ...
    await supabase.from('movimenti_cassa').insert([{
      sala_id: currentSalaId, tipo: 'entrata', categoria: 'biliardo_bar',
      metodo_pagamento: metodo === 'credito' ? 'credito_vip' : metodo,
      importo: summaryData.totale.toFixed(2), descrizione: `Incasso ${summaryData.nome}`, staff_id: staffId
    }]);
  ...
  const salvaUscita = async (staffId: string) => {
    ...
    await supabase.from('movimenti_cassa').insert([{
      sala_id: currentSalaId, tipo: 'uscita', categoria: 'spese_varie',
      metodo_pagamento: uscitaMetodo, importo: parseFloat(uscitaImporto).toFixed(2),
      descrizione: uscitaDescrizione, staff_id: staffId
    }]);
    await refreshDati(currentSalaId!); 
  ...
  const inviaLinkWhatsApp = (socio: any) => {
    ...
    const messaggioTesto = `Ciao ${socio.nome}, ecco la tua Tessera Digitale VIP per ${nomeSala}.
    Clicca qui per vedere il tuo credito e prenotare: ${url}`;
  ...
  const esportaCSV = () => {
    ...
    } catch (error) { alert("❌ C'è stato un problema durante l'esportazione."); }
  };
  ...
  const scaricaPrimaNotaPDF = () => {
    ...
    doc.setFontSize(10); doc.setTextColor(100, 100, 100); doc.text(`Data e Ora: ${dataOggi} - ${oraOggi}`, 14, 38);
    doc.text(`Operatore: ${operatore}`, 14, 44);

    doc.setFontSize(11); doc.setTextColor(0, 0, 0); doc.setFont('helvetica', 'bold'); doc.text("RIEPILOGO GIORNALIERO", 14, 55);
    doc.setFont('helvetica', 'normal'); doc.text(`Totale Entrate: € ${incassoTotale.toFixed(2)}`, 14, 62);
    doc.text(`Totale Uscite: € ${usciteTotali.toFixed(2)}`, 14, 68); doc.text(`Incasso POS: € ${incassoPOS.toFixed(2)}`, 100, 62);
    doc.setFontSize(12); doc.setTextColor(6, 182, 212); doc.setFont('helvetica', 'bold'); doc.text(`SALDO CASSETTO: € ${incassoContanti.toFixed(2)}`, 100, 68);
    ...
    autoTable(doc, {
      startY: 75, head: [colonne], body: righe, theme: 'grid', headStyles: { fillColor: [22, 163, 74], textColor: 255 }, styles: { fontSize: 9 },
    ...
    doc.text("Firma Operatore ________________________", 14, finalY + 20); doc.text("Firma Gestore ________________________", 120, finalY + 20);
    doc.save(nomeFile);
  };

  const esportaStorico = async (formato: 'csv' | 'pdf') => {
    if (!storicoDal || !storicoAl) { alert("⚠️ Seleziona sia la Data di Inizio che la Data di Fine!"); return; }
    ...
    if (dataInizio > dataFine) { alert("⚠️ La Data di Inizio non può essere successiva alla Data di Fine!"); return; }
    ...
    try {
      ...
      if (error || !data || data.length === 0) { alert("Nessun movimento trovato nel periodo selezionato."); return; }
      ...
      if (formato === 'csv') {
        ...
        const blob = new Blob(["\uFEFF" + csvContent], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `Storico_Cassa_${storicoDal}_al_${storicoAl}.csv`;
        document.body.appendChild(link); link.click(); document.body.removeChild(link);
        ...
        doc.setFontSize(22); doc.setTextColor(37, 99, 235); doc.text(`STORICO INCASSI: ${nomeSala.toUpperCase()}`, 14, 20);
        doc.setFontSize(12); doc.setTextColor(100); doc.text(`Periodo analizzato: dal ${new Date(storicoDal).toLocaleDateString('it-IT')} al ${new Date(storicoAl).toLocaleDateString('it-IT')}`, 14, 30);
        ...
        doc.text(`Totale Entrate Reali: € ${entrate.toFixed(2)}`, 14, 45); doc.text(`Totale Uscite: € ${uscite.toFixed(2)}`, 14, 52);
        doc.setTextColor(37, 99, 235); doc.text(`Utile Netto del Periodo: € ${(entrate - uscite).toFixed(2)}`, 14, 59);
        ...
        autoTable(doc, {
          startY: 65, head: [colonne], body: righe, theme: 'grid', headStyles: { fillColor: [37, 99, 235], textColor: 255 }, styles: { fontSize: 8 },
          columnStyles: { 4: { halign: 'right', fontStyle: 'bold' } },
          didParseCell: function(d:any) { 
            if(d.column.index === 4 && d.section === 'body') { d.cell.styles.textColor = d.cell.raw.toString().startsWith('+') ? [22,163,74] : [220,38,38]; }
          }
        });
        doc.save(nomeFile);
      }
    } catch (error) { alert("❌ Errore durante l'estrazione dello storico."); }
  };
  ...
  const scaricaSociPDF = () => {
    ...
    autoTable(doc, {
      startY: 55, head: [colonne], body: righe, theme: 'grid', headStyles: { fillColor: [202, 138, 4], textColor: 255 }, styles: { fontSize: 10 },
    ...
    <div className="min-h-screen bg-black text-white p-4 font-sans tracking-tighter overflow-x-hidden relative print:bg-white print:text-black">
      ...
      <div className={`print:hidden ${isSalaSuspended ? 'pt-16' : ''}`}>
          ...
          <div className="animate-in fade-in duration-500 text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-6xl mx-auto">
              ...
              <button onClick={() => setActiveView("prenotazioni")} className="bg-gray-900 border-2 border-teal-600 p-8 rounded-[2.5rem] shadow-2xl hover:bg-gray-800 transition-all relative">
                ...
                {pendingPrenotazioni.length > 0 && !isSalaSuspended && (
                  <span className="absolute -top-3 -right-3 bg-red-600 text-white font-black rounded-full w-10 h-10 flex items-center justify-center border-2 border-black animate-pulse">{pendingPrenotazioni.length}</span>
                )}
              </button>
            ...
            <div className="space-y-6">
              {bachecaPosts.length === 0 ? (
                ...
                bachecaPosts.map((post) => {
                  ...
                  const conteggio = reazioni.reduce((acc: any, curr: any) => { acc[curr.tipo] = (acc[curr.tipo] || 0) + 1; return acc; }, {});
                  return (
                      ...
                      <div className="flex flex-wrap gap-2 border-t border-gray-800 pt-4">
                        {Object.entries(conteggio).length === 0 ? (
                          ...
                          Object.entries(conteggio).map(([emoji, count]) => (
                            <span key={emoji} className="bg-gray-900 px-3 py-1 rounded-full text-sm border border-gray-700">{emoji} <span className="font-bold text-white ml-1">{count as number}</span></span>
                          ))
                        )}
                      </div>
                    </div>
          ...
          <div className="max-w-6xl mx-auto animate-in slide-in-from-bottom-8">
            {getPrenotazioniConfermateOggi().length > 0 && (
              <div className="mb-8 bg-teal-900/30 border-2 border-teal-600 rounded-3xl p-6 shadow-lg animate-in fade-in">
                <h3 className="text-teal-400 font-black uppercase mb-4 flex items-center gap-2 tracking-widest text-sm"><span className="text-xl">📅</span> Prenotazioni Confermate in arrivo oggi</h3>
                <div className="flex gap-4 overflow-x-auto pb-2 custom-scrollbar">
                  {getPrenotazioniConfermateOggi().map(p => (
                    <div key={p.id} className="min-w-[250px] bg-black p-4 rounded-2xl border border-teal-800 flex flex-col justify-between">
                      <div><p className="font-black text-white uppercase truncate">{p.nome_cliente}</p><p className="text-teal-500 font-mono text-2xl font-black">{new Date(p.data_ora).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {tavoli.map((t) => (
                <div key={t.id} className={`p-8 rounded-[2.5rem] border-4 transition-colors shadow-2xl ${t.stato === 'IN GIOCO' ? 'border-red-600 bg-gray-900' : t.stato === 'PRENOTATO' ? 'border-yellow-500 bg-yellow-900/30' : 'border-green-900 bg-gray-950'}`}>
                  ...
                  {isSalaSuspended && t.stato === 'IN GIOCO' && (
                    <div className="text-center py-4 bg-red-950/30 rounded-2xl border border-red-900 text-red-500 font-bold uppercase text-xs">Sospeso: Operazioni disabilitate</div>
                  )}
                </div>
              ))}
            </div>
                ...
                <div key={p.id} className="bg-gray-900 p-6 rounded-[2rem] border-2 border-gray-800 text-center flex flex-col justify-between shadow-xl hover:border-gray-700 transition-colors">
                  ...
                  {!isSalaSuspended && (
                    <div className="flex gap-2 mt-auto border-t border-gray-800 pt-4">
                      <button onClick={() => apriModificaProdotto(p)} className="flex-1 bg-blue-900/50 border border-blue-800 text-blue-400 hover:bg-blue-600 hover:text-white py-2 rounded-xl text-[10px] font-black uppercase transition-colors">✏️ Modifica</button>
                      <button onClick={() => richiedePin((sid) => eliminaProdotto(p.id, sid), "Elimina Prodotto")} className="flex-1 bg-red-950/50 border border-red-900 text-red-500 hover:bg-red-600 hover:text-white py-2 rounded-xl text-[10px] font-black uppercase transition-colors">🗑️ Elimina</button>
                    </div>
                  )}
                  {isSalaSuspended && (<div className="mt-auto border-t border-gray-800 pt-4 text-gray-600 text-[10px] font-bold uppercase tracking-widest">Sola Lettura</div>)}
                </div>
              ))}
            </div>
          </div>
        ...
        {activeView === 'soci' && (
          <div className="max-w-6xl mx-auto animate-in slide-in-from-bottom-8">
            <div className="flex flex-col md:flex-row gap-4 mb-8">
              {!isSalaSuspended && (<button onClick={() => setIsNewSocioModalOpen(true)} className="flex-[2] py-8 bg-yellow-600 text-black font-black text-2xl uppercase shadow-xl rounded-[2rem] hover:bg-yellow-500 transition-colors">+ NUOVO SOCIO</button>)}
              <button onClick={scaricaSociPDF} className="flex-1 py-8 bg-gray-900 border-2 border-yellow-500 text-yellow-500 font-black text-xl uppercase shadow-xl rounded-[2rem] hover:bg-yellow-600 hover:text-black transition-all flex items-center justify-center gap-3 active:scale-95"><span>🖨️</span> STAMPA ELENCO (PDF)</button>
            </div>
            <div className="bg-gray-900 border-2 border-gray-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
              <table className="w-full text-left font-bold">
                <thead className="bg-gray-800 text-gray-400 uppercase text-xs"><tr><th className="p-6">Socio</th><th className="p-6">Credito Attuale</th><th className="p-6 text-center">App Personale</th><th className="p-6 text-right">Azione</th></tr></thead>
                <tbody className="divide-y divide-gray-800">
                  {soci.map((s) => (
                    <tr key={s.id} className="hover:bg-gray-800/30 transition-all">
                      <td className="p-6 text-xl uppercase italic">{s.cognome} {s.nome}{s.telefono && <p className="text-gray-500 text-xs mt-1 font-mono">{s.telefono}</p>}</td>
                      <td className="p-6 text-2xl text-green-500 italic">€ {parseFloat(s.credito || 0).toFixed(2)}</td>
                      <td className="p-6 text-center"><button onClick={() => inviaLinkWhatsApp(s)} className="bg-green-900/50 border border-green-700 text-green-300 px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-green-600 hover:text-white transition-all shadow-md flex items-center justify-center gap-2 mx-auto"><span className="text-lg">💬</span> Invia WhatsApp</button></td>
                      <td className="p-6">
                        {!isSalaSuspended ? (
                          <div className="flex justify-end items-center gap-2">
                            <button onClick={() => apriModificaSocio(s)} className="bg-blue-900/50 border border-blue-700 text-blue-300 px-4 py-3 rounded-xl text-xs font-black uppercase hover:bg-blue-600 hover:text-white transition-all shadow-md">✏️ Modifica</button>
                            <button onClick={() => { setSocioToRecharge(s); setIsRechargeModalOpen(true); }} className="bg-green-600 text-black px-4 py-3 rounded-xl text-xs font-black uppercase shadow-md hover:bg-green-500 transition-all">💰 Ricarica</button>
                            <button onClick={() => richiedePin((sid) => eliminaSocio(s.id, sid), "Elimina Socio")} className="bg-red-950/80 border border-red-800 text-red-500 px-4 py-3 rounded-xl text-xs font-black uppercase hover:bg-red-600 hover:text-white transition-all shadow-md">🗑️ Elimina</button>
                          </div>
        ...
        {activeView === 'report' && (
          <div className="max-w-6xl mx-auto animate-in slide-in-from-bottom-8 text-center">
            {!isSalaSuspended && (<button onClick={() => setIsNewUscitaModalOpen(true)} className="w-full mb-8 py-8 bg-red-600 text-white font-black text-2xl uppercase shadow-xl rounded-[2rem] hover:bg-red-500 transition-colors">- REGISTRA SPESA / USCITA CASSA</button>)}
            <div className="flex flex-col md:flex-row gap-4 mb-12">
              <button onClick={scaricaPrimaNotaPDF} className="flex-1 py-6 bg-gray-900 border-2 border-green-500 text-green-400 font-black text-xl uppercase shadow-xl rounded-[2rem] hover:bg-green-600 hover:text-black transition-all flex items-center justify-center gap-3 active:scale-95"><span>🖨️</span> STAMPA PRIMA NOTA (PDF)</button>
              <button onClick={esportaCSV} className="flex-1 py-6 bg-gray-900 border-2 border-blue-500 text-blue-400 font-black text-xl uppercase shadow-xl rounded-[2rem] hover:bg-blue-600 hover:text-black transition-all flex items-center justify-center gap-3 active:scale-95"><span>📥</span> SCARICA (EXCEL / CSV)</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
              ...
              <div className="bg-cyan-950 p-8 rounded-[3rem] border-4 border-cyan-500 shadow-[0_0_30px_rgba(6,182,212,0.2)] md:col-span-2"><p className="text-cyan-400 font-black uppercase text-[10px] tracking-widest mb-2">Saldo Cassetto (Contanti Reali)</p><h3 className="text-5xl font-black text-cyan-300 italic">€ {incassoContanti.toFixed(2)}</h3><p className="text-gray-500 text-xs mt-2 uppercase font-bold tracking-widest">In POS/Banca: € {incassoPOS.toFixed(2)}</p></div>
            </div>
                  ...
                  {datiGrafico.map((g, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex flex-col items-center flex-1 group h-full justify-end">
                        ...
                        <div className="w-full max-w-[50px] bg-green-900/30 group-hover:bg-green-500 transition-colors rounded-t-xl border-b-4 border-green-500 relative" style={{ height: altezza, minHeight: '8px' }}><div className="absolute bottom-full left-0 w-full bg-green-400/20 rounded-t-xl transition-all" style={{height: '100%'}}></div></div>
            ...
            <div className="bg-gray-900 border-2 border-gray-800 rounded-[3rem] p-8 mb-12 shadow-2xl">
              ...
              <div className="flex flex-col md:flex-row gap-4 items-center">
                <div className="flex-1 w-full"><label className="block text-gray-500 text-xs font-black uppercase mb-2 text-left">Data Inizio</label><input type="date" value={storicoDal} onChange={(e) => setStoricoDal(e.target.value)} className="w-full bg-black border border-gray-800 p-4 rounded-2xl text-white outline-none focus:border-blue-500 transition-colors" /></div>
                <div className="flex-1 w-full"><label className="block text-gray-500 text-xs font-black uppercase mb-2 text-left">Data Fine</label><input type="date" value={storicoAl} onChange={(e) => setStoricoAl(e.target.value)} className="w-full bg-black border border-gray-800 p-4 rounded-2xl text-white outline-none focus:border-blue-500 transition-colors" /></div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-[2.5rem] overflow-hidden text-left shadow-2xl">
              <div className="p-6 bg-gray-800 border-b border-gray-700 text-center"><h3 className="text-white font-black uppercase tracking-widest">Prima Nota Contabile (Oggi)</h3></div>
              <table className="w-full text-xs uppercase font-bold">
                <thead className="bg-gray-800 text-gray-500"><tr><th className="p-5">Ora</th><th className="p-5">Causale</th><th className="p-5">Staff</th><th className="p-5 text-center">Metodo</th><th className="p-5 text-right">Importo</th><th className="p-5 text-center">Storno</th></tr></thead>
                <tbody className="divide-y divide-gray-800">
                  {primaNota.length === 0 ? (
                    ...
                    primaNota.map((m) => (
                      <tr key={m.id} className="hover:bg-gray-800/20 transition-all">
                        ...
                        <td className="p-5 text-white">{m.descrizione}</td><td className="p-5 text-gray-500">{m.staff?.nome || "ADMIN"}</td>
                        <td className="p-5 text-center"><span className={`px-3 py-1 rounded-lg text-[10px] ${m.metodo_pagamento === 'contanti' ? 'bg-cyan-900/50 text-cyan-400' : m.metodo_pagamento === 'pos' ? 'bg-blue-900/50 text-blue-400' : 'bg-yellow-900/50 text-yellow-400'}`}>{m.metodo_pagamento.replace('_', ' ')}</span></td>
                        <td className={`p-5 text-right font-black text-lg italic ${m.tipo === 'entrata' ? 'text-green-500' : 'text-red-500'}`}>{m.tipo === 'entrata' ? '+' : '-'} € {parseFloat(m.importo).toFixed(2)}</td>
                        <td className="p-5 text-center">{!isSalaSuspended && (<button onClick={() => richiedePin((sid) => stornoMovimento(m.id, sid), "Storno Movimento")} className="text-gray-600 hover:text-red-500 text-xl transition-colors">✕</button>)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
        ...
        {activeView === 'impostazioni' && (
          <div className="max-w-2xl mx-auto bg-gray-900 p-10 rounded-[3rem] border-4 border-gray-800 animate-in slide-in-from-bottom-8 shadow-2xl">
            ...
            {isSalaSuspended && (<p className="text-red-500 font-black uppercase text-xs">Modifica Tariffe disabilitata in Sola Lettura</p>)}
            <div className="mt-12 pt-8 border-t border-gray-800">
              ...
              <div className="bg-black p-6 rounded-[2rem] border border-pink-900/30 flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="text-center md:text-left"><p className="font-bold text-white uppercase tracking-widest text-sm">Accesso Tecnico Remoto</p><p className="text-xs text-gray-500 mt-1">Consenti al Super Admin di accedere per assistenza.</p></div>
                <button disabled={isSalaSuspended} onClick={async () => { const nuovoStato = !supportActive; const { error } = await supabase.from('sale').update({ support_active: nuovoStato }).eq('id', currentSalaId); if (!error) { setSupportActive(nuovoStato); alert(nuovoStato ? "✅ Assistenza Attivata" : "🔒 Assistenza Disattivata"); } }} className={`px-8 py-4 rounded-2xl font-black text-sm uppercase transition-all ${isSalaSuspended ? 'bg-gray-800 text-gray-500' : supportActive ? 'bg-red-600 text-white' : 'bg-green-600 text-white'}`}>
        ...
        {activeView === 'prenotazioni' && (
          <div className="max-w-6xl mx-auto animate-in slide-in-from-bottom-8">
            ...
            <div className="flex justify-center mb-8"><button onClick={scaricaPrenotazioniPDF} className="w-full md:w-auto px-10 py-6 bg-gray-900 border-2 border-teal-500 text-teal-400 font-black text-xl uppercase shadow-xl rounded-[2rem] hover:bg-teal-600 hover:text-black transition-all flex items-center justify-center gap-3 active:scale-95"><span>🖨️</span> STAMPA FOGLIO DI MARCIA (PDF)</button></div>
            ...
            {getPrenotazioniFiltrate().length === 0 ? (
              <div className="bg-gray-900 p-10 rounded-[3rem] border-2 border-gray-800 shadow-2xl text-center"><p className="text-gray-500 font-bold uppercase tracking-widest text-lg">Nessuna prenotazione attiva al momento.</p></div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  return (
                    <div key={p.id} className={`border-2 p-6 rounded-3xl shadow-xl flex flex-col justify-between transition-colors ${isDaImpostare ? 'bg-gray-900 border-teal-600' : 'bg-gray-950 border-gray-800'}`}>
                      <div>
                        ...
                        <div className="mb-4"><p className="text-teal-500 font-bold uppercase text-xs tracking-widest">Data e Ora</p><p className="text-xl font-mono font-black text-white">{dataPrenotazione.toLocaleDateString()} - {dataPrenotazione.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p></div>
                        <div className="mb-6"><p className="text-gray-500 font-bold uppercase text-xs tracking-widest">Telefono / Note</p><p className="text-md text-gray-300 font-bold">{p.telefono}</p>{p.note && <p className="text-sm text-gray-400 italic mt-1 bg-black p-2 rounded-lg border border-gray-800">{p.note}</p>}</div>
                      </div>
                      <div className="flex gap-2 mt-auto">
                        {isDaImpostare ? (
                          <>
                            {!isSalaSuspended ? (
                              <><button onClick={() => richiedePin((sid) => gestisciStatoPrenotazione(p.id, 'confermata', sid), "Conferma Prenotazione")} className="flex-[2] bg-teal-600 text-black font-black uppercase py-3 rounded-xl hover:bg-teal-500 transition-colors shadow-lg">Conferma</button><button onClick={() => richiedePin((sid) => gestisciStatoPrenotazione(p.id, 'rifiutata', sid), "Rifiuta Prenotazione")} className="flex-[1] bg-gray-800 text-red-500 font-black uppercase py-3 rounded-xl hover:bg-red-900 hover:text-white transition-colors">Rifiuta</button></>
                            ) : (<div className="w-full text-center py-3 bg-black rounded-xl border border-gray-800 text-red-600 font-black uppercase text-xs tracking-widest">AZIONE BLOCCATA</div>)}
                          </>
                        ) : (<div className="w-full text-center py-3 bg-black rounded-xl border border-gray-800 text-gray-600 font-black uppercase text-xs tracking-widest">{p.stato === 'confermata' ? 'PRENOTAZIONE CONFERMATA' : 'PRENOTAZIONE RIFIUTATA'}</div>)}
                      </div>
                    </div>
                  );
                })}
        ...
        {activeView === 'tornei' && (
          <div className="max-w-6xl mx-auto animate-in slide-in-from-bottom-8">
            ...
            {tornei.length === 0 ? (
              <div className="bg-gray-900 p-10 rounded-[3rem] border-4 border-gray-800 shadow-2xl text-center"><p className="text-gray-500 font-bold uppercase">Nessun torneo programmato. Creane uno!</p></div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {tornei.map((tr) => (
                  <div key={tr.id} className="bg-gray-900 p-8 rounded-[2.5rem] border-2 border-pink-900 shadow-2xl flex flex-col justify-between relative overflow-hidden">
                    <div className={`absolute top-0 right-0 w-32 py-1 text-center font-black text-[10px] uppercase tracking-widest translate-x-8 translate-y-4 rotate-45 ${tr.stato === 'iscrizioni' ? 'bg-yellow-500 text-black' : tr.stato === 'in_corso' ? 'bg-blue-600 text-white' : 'bg-gray-500 text-white'}`}>{tr.stato === 'iscrizioni' ? 'Aperto' : tr.stato === 'in_corso' ? 'Live' : 'Chiuso'}</div>
                    <div><h4 className="text-2xl font-black uppercase text-white italic mb-2">{tr.nome}</h4><p className="text-pink-400 font-mono font-bold text-lg mb-2">📅 Data: {new Date(tr.data_inizio).toLocaleDateString()}</p><p className="text-green-500 font-bold mb-6">💰 Quota: € {parseFloat(tr.quota_iscrizione).toFixed(2)}</p></div>
                    <div className="flex flex-col gap-2">
                      ...
                      {tr.stato === 'in_corso' && (<button onClick={() => { setActiveTorneo(tr); setIsBracketModalOpen(true); }} className="w-full py-4 bg-blue-600 text-white font-black uppercase rounded-2xl hover:bg-blue-500 transition-all shadow-lg flex justify-center items-center gap-2"><span>🏆</span> APRI TABELLONE LIVE</button>)}
                      {tr.stato === 'completato' && (<button onClick={() => { setActiveTorneo(tr); setIsBracketModalOpen(true); }} className="w-full py-4 bg-gray-700 text-white font-black uppercase rounded-2xl hover:bg-gray-600 transition-all flex justify-center items-center gap-2"><span>📜</span> RISULTATI FINALI</button>)}
                      <button onClick={() => { const urlBacheca = `${window.location.origin}/bacheca/${currentSalaId}/${tr.id}`; alert(`Invia questo link ai giocatori per far loro seguire il tabellone in diretta sul cellulare:\n\n${urlBacheca}`); }} className="w-full py-3 bg-gray-900 border border-gray-700 text-gray-400 font-black uppercase rounded-2xl hover:text-white transition-all text-xs mt-2">🔗 LINK PER I SOCI (CELLULARI)</button>
        ...
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-[200] animate-in zoom-in-95 print:hidden">
          <div className="w-full max-w-[320px] bg-[#0B1021] border-4 border-pink-500 p-8 rounded-[3rem] shadow-[0_0_40px_rgba(236,72,153,0.2)] text-center relative">
            <div className="flex justify-center gap-4 mb-4">
              {[...Array(4)].map((_, i) => (<div key={i} className={`w-6 h-6 rounded-full border-2 border-cyan-400 ${pinBuffer.length > i ? 'bg-cyan-400 shadow-[0_0_15px_#22d3ee]' : 'bg-transparent'}`}></div>))}
            </div>
      ...
      {isNewUscitaModalOpen && !isSalaSuspended && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 animate-in zoom-in-95 print:hidden"><div className="bg-gray-900 border-4 border-red-600 p-10 rounded-[3rem] w-full max-w-lg text-center shadow-2xl"><h3 className="text-3xl font-black text-red-500 mb-8 uppercase italic">Registra Spesa</h3><input value={uscitaDescrizione} onChange={(e)=>setUscitaDescrizione(e.target.value)} placeholder="Causale (es. Fornitore, Pulizie...)" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-xl text-white mb-4 outline-none text-center focus:border-red-500" /><input type="number" value={uscitaImporto} onChange={(e)=>setUscitaImporto(e.target.value)} placeholder="Importo (€)" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-4xl font-mono text-red-400 mb-4 text-center outline-none focus:border-red-500" /><select value={uscitaMetodo} onChange={(e)=>setUscitaMetodo(e.target.value)} className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-xl text-white mb-8 outline-none text-center focus:border-red-500"><option value="contanti">Prelevati in Contanti</option><option value="pos">Pagati con Carta/Dal Conto</option></select><button onClick={() => richiedePin((sid) => salvaUscita(sid), "Registrazione Uscita")} className="w-full py-8 bg-red-600 text-white font-black uppercase text-xl rounded-3xl shadow-xl active:scale-95 transition-colors">CONFERMA USCITA</button><button onClick={()=>setIsNewUscitaModalOpen(false)} className="w-full py-4 text-gray-500 uppercase font-bold mt-4">Annulla</button></div></div>)}
      {isRechargeModalOpen && socioToRecharge && !isSalaSuspended && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 animate-in fade-in print:hidden"><div className="bg-gray-900 border-4 border-green-600 p-10 rounded-[3rem] w-full max-w-lg text-center shadow-2xl"><h3 className="text-2xl font-black text-green-500 mb-2 uppercase italic tracking-tighter">Ricarica Credito</h3><p className="text-3xl font-black text-white mb-8 uppercase italic">{socioToRecharge.nome} {socioToRecharge.cognome}</p><input type="number" value={rechargeAmount} onChange={(e)=>setRechargeAmount(e.target.value)} placeholder="€" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-6xl text-center text-green-500 mb-4 outline-none font-black focus:border-green-500" /><select value={rechargeMetodo} onChange={(e)=>setRechargeMetodo(e.target.value)} className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-lg text-white mb-8 outline-none text-center focus:border-green-500"><option value="contanti">Il socio mi ha dato Contanti</option><option value="pos">Il socio ha pagato col POS</option></select><button onClick={() => richiedePin((sid) => salvaRicarica(sid), `Ricarica ${socioToRecharge.nome}`)} className="w-full py-8 bg-green-600 text-black rounded-3xl font-black uppercase text-xl shadow-xl active:scale-95 transition-all">CONFERMA CON PIN</button><button onClick={()=>setIsRechargeModalOpen(false)} className="w-full py-4 text-gray-500 uppercase font-bold mt-4">Annulla</button></div></div>)}
      ...
      {isEditProductModalOpen && !isSalaSuspended && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 animate-in zoom-in-95 print:hidden"><div className="bg-gray-900 border-4 border-blue-600 p-10 rounded-[3rem] w-full max-w-lg shadow-2xl text-center"><h3 className="text-3xl font-black text-blue-500 mb-8 uppercase italic">Modifica Prodotto</h3><input value={editProdName} onChange={(e) => setEditProdName(e.target.value)} placeholder="Nome Prodotto" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-xl text-white mb-4 outline-none text-center focus:border-blue-500 transition-all" /><input type="number" value={editProdPrice} onChange={(e) => setEditProdPrice(e.target.value)} placeholder="Prezzo (€)" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-xl text-white mb-4 outline-none text-center focus:border-blue-500 transition-all" /><input type="number" value={editProdStock} onChange={(e) => setEditProdStock(e.target.value)} placeholder="Quantità in Stock" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-xl text-white mb-8 outline-none text-center focus:border-blue-500 transition-all" /><button onClick={() => richiedePin((sid) => salvaModificaProdotto(sid), "Modifica Magazzino")} className="w-full py-8 bg-blue-600 text-white font-black uppercase text-xl rounded-3xl shadow-xl active:scale-95 transition-all">SALVA MODIFICHE</button><button onClick={()=>setIsEditProductModalOpen(false)} className="w-full py-4 text-gray-500 uppercase font-bold mt-4 text-center">Annulla</button></div></div>)}
      ...
      {isEditSocioModalOpen && !isSalaSuspended && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 animate-in zoom-in-95 print:hidden"><div className="bg-gray-900 border-4 border-blue-600 p-10 rounded-[3rem] w-full max-w-lg shadow-2xl animate-in zoom-in-95 text-center"><h3 className="text-3xl font-black text-blue-500 mb-8 uppercase italic">Modifica Dati Socio</h3><input value={editSocioNome} onChange={(e) => setEditSocioNome(e.target.value)} placeholder="Nome" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-xl text-white mb-4 outline-none text-center focus:border-blue-500 transition-all" /><input value={editSocioCognome} onChange={(e) => setEditSocioCognome(e.target.value)} placeholder="Cognome" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-xl text-white mb-4 outline-none text-center focus:border-blue-500 transition-all" /><input value={editSocioTelefono} onChange={(e) => setEditSocioTelefono(e.target.value)} placeholder="Telefono (es. 3331234567)" type="tel" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-xl text-white mb-8 outline-none text-center focus:border-blue-500 transition-all" /><button onClick={() => richiedePin((sid) => salvaModificaSocio(sid), "Modifica Socio")} className="w-full py-8 bg-blue-600 text-white rounded-3xl font-black uppercase text-xl shadow-xl active:scale-95 transition-all">SALVA MODIFICHE</button><button onClick={()=>setIsEditSocioModalOpen(false)} className="w-full py-4 text-gray-500 uppercase font-bold mt-4 text-center">Annulla</button></div></div>)}
      ...
      {isEditTableModalOpen && !isSalaSuspended && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 animate-in zoom-in-95 print:hidden"><div className="bg-gray-900 border-4 border-blue-600 p-10 rounded-[3rem] w-full max-w-lg text-center shadow-2xl"><h3 className="text-3xl font-black text-blue-500 mb-8 uppercase italic">Modifica Tavolo</h3><label className="block text-gray-500 font-bold text-xs uppercase mb-2">Numero Tavolo</label><input type="number" value={editTableNumber} onChange={(e)=>setEditTableNumber(e.target.value)} className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-6xl text-center text-white mb-8 outline-none focus:border-blue-500" /><button onClick={() => richiedePin((sid) => salvaModificaTavolo(sid), "Modifica Tavolo")} className="w-full py-8 bg-blue-600 text-white font-black uppercase text-xl rounded-3xl shadow-xl active:scale-95 transition-colors">SALVA MODIFICHE</button><button onClick={()=>setIsEditTableModalOpen(false)} className="w-full py-4 text-gray-500 uppercase font-bold mt-4">Annulla</button></div></div>)}
      ...
      {isManageIscrittiOpen && activeTorneo && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 animate-in zoom-in-95 print:hidden"><div className="bg-gray-950 border-4 border-pink-600 p-8 rounded-[3rem] w-full max-w-6xl shadow-2xl flex flex-col h-[90vh] relative"><button onClick={() => { setIsManageIscrittiOpen(false); setActiveTorneo(null); }} className="absolute top-6 right-6 text-gray-500 hover:text-white text-3xl font-black transition-colors z-20 bg-black hover:bg-red-600 w-16 h-16 rounded-full flex items-center justify-center border-4 border-gray-700 shadow-2xl">✕</button><h3 className="text-4xl font-black text-pink-500 mb-2 uppercase italic text-center pr-16">{activeTorneo.nome}</h3><p className="text-gray-400 text-center font-bold mb-8 uppercase tracking-widest text-sm">Gestione Iscritti (Totale: {(activeTorneo.iscritti || []).length})</p><div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 overflow-hidden"><div className="lg:col-span-7 flex flex-col bg-black rounded-[2rem] border-2 border-gray-800 overflow-hidden"><div className="bg-gray-900 p-4 border-b border-gray-800 text-center font-black uppercase text-gray-500 tracking-widest text-xs">Elenco Partecipanti</div><div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">{(activeTorneo.iscritti || []).length === 0 ? (<p className="text-center text-gray-600 font-bold uppercase mt-10">Nessun giocatore iscritto.</p>) : (normalizeIscritti(activeTorneo.iscritti).map((iscritto: any, index: number) => (<div key={iscritto.id} className="flex justify-between items-center p-4 rounded-2xl border border-gray-800 bg-gray-900"><div className="flex items-center gap-4"><span className="text-pink-600 font-black text-xl w-6">{index + 1}.</span><span className="text-white font-black text-xl uppercase italic">{iscritto.nome}</span><span className={`text-[10px] px-3 py-1 rounded uppercase font-black tracking-widest ${iscritto.tipo === 'socio' ? 'bg-pink-600 text-white' : 'bg-purple-600 text-white'}`}>{iscritto.tipo}</span></div>{!isSalaSuspended && (<button onClick={() => richiedePin((sid) => rimuoviIscritto(iscritto.id, sid), "Annulla Iscrizione")} className="bg-red-950 text-red-500 p-3 rounded-xl hover:bg-red-900 transition-colors">✕</button>)}</div>)))}</div></div><div className="lg:col-span-5 flex flex-col gap-6 overflow-y-auto pr-2">{!isSalaSuspended && (<><div className="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 flex flex-col justify-center"><p className="text-purple-400 font-black uppercase text-xs tracking-widest text-center mb-4">Aggiungi Giocatore Esterno</p><div className="flex flex-col gap-4"><input type="text" value={iscrittoEsterno} onChange={(e) => setIscrittoEsterno(e.target.value)} placeholder="Nome e Cognome..." className="w-full bg-black border border-gray-700 p-4 rounded-xl text-lg text-white outline-none focus:border-purple-500 transition-all text-center" /><button onClick={() => richiedePin((sid) => aggiungiIscritto(sid, 'esterno'), "Iscrizione Esterno")} className="w-full py-4 bg-purple-600 text-white font-black uppercase rounded-xl hover:bg-purple-500 active:scale-95 transition-all text-lg">➕ ESTERNO</button></div></div><div className="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 flex flex-col justify-center"><p className="text-pink-400 font-black uppercase text-xs tracking-widest text-center mb-4">Aggiungi Socio Tesserato</p><div className="flex flex-col gap-4"><select value={iscrittoSelezionato} onChange={(e) => setIscrittoSelezionato(e.target.value)} className="w-full bg-black border border-gray-700 p-4 rounded-xl text-lg text-white outline-none focus:border-pink-500 transition-all text-center"><option value="">Seleziona un socio...</option>{soci.map(s => { const current = normalizeIscritti(activeTorneo.iscritti); if (current.find((i:any) => i.id === s.id)) return null; return <option key={s.id} value={s.id}>{s.cognome} {s.nome}</option> })}</select><button onClick={() => richiedePin((sid) => aggiungiIscritto(sid, 'socio'), "Iscrizione Socio")} className="w-full py-4 bg-pink-600 text-white font-black uppercase rounded-xl hover:bg-pink-500 active:scale-95 transition-all text-lg">➕ SOCIO</button></div></div></>)}{isSalaSuspended && (<div className="text-center py-10 bg-red-950/20 rounded-3xl border border-red-900"><p className="text-red-500 font-bold uppercase tracking-widest">Aggiunta giocatori disabilitata (Sola Lettura)</p></div>)}</div></div><button onClick={() => { setIsManageIscrittiOpen(false); setActiveTorneo(null); }} className="w-full py-6 mt-8 bg-gray-800 text-white uppercase font-black rounded-3xl hover:bg-gray-700 transition-all tracking-widest">CHIUDI GESTIONE</button></div></div>)}
      {isBracketModalOpen && activeTorneo && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 animate-in zoom-in-95 print:relative print:bg-white print:text-black print:p-0 print:block"><div className="bg-gray-950 border-4 border-blue-600 p-8 rounded-[3rem] w-full max-w-[95vw] shadow-2xl flex flex-col max-h-[95vh] relative print:border-none print:shadow-none print:bg-white print:max-h-none print:h-auto print:overflow-visible"><button onClick={() => { setIsBracketModalOpen(false); setActiveTorneo(null); }} className="absolute top-6 right-6 text-gray-500 hover:text-white text-3xl font-black transition-colors z-20 bg-black hover:bg-red-600 w-16 h-16 rounded-full flex items-center justify-center border-4 border-gray-700 shadow-2xl print:hidden">✕</button><button onClick={eseguiStampa} className="absolute top-6 left-6 bg-yellow-500 text-black font-black uppercase tracking-widest px-6 py-4 rounded-2xl hover:bg-yellow-400 transition-all shadow-xl z-20 print:hidden flex items-center gap-2">🖨️ Stampa Tabellone</button><h3 className="text-5xl font-black text-blue-500 mb-2 uppercase italic text-center print:text-black">{activeTorneo.nome}</h3><p className="text-gray-400 text-center font-bold mb-8 uppercase tracking-widest print:text-gray-600">{activeTorneo.stato === 'completato' ? '🏆 TABELLONE FINALE 🏆' : 'SCONTRI DIRETTI IN CORSO'}</p><div className="flex-1 overflow-x-auto overflow-y-auto bg-black p-8 rounded-3xl border-4 border-gray-900 shadow-inner custom-scrollbar relative print:bg-white print:border-none print:shadow-none print:overflow-visible"><div className="flex flex-row min-w-max h-full min-h-[500px] gap-12 print:gap-8">{activeTorneo.tabellone?.map((turno: any, turnoIndex: number) => (<div key={turnoIndex} className="flex flex-col justify-around w-72 relative print:w-48"><div className="absolute -top-4 w-full text-center border-b-2 border-gray-800 pb-2 print:border-gray-300"><span className="bg-blue-900/50 text-blue-400 font-black uppercase tracking-widest px-4 py-1 rounded-lg text-xs print:bg-gray-200 print:text-black print:border print:border-black">Turno {turnoIndex + 1}</span></div>{turno.map((match: any) => (<div key={match.id} className="relative w-full bg-gray-900 border border-gray-700 rounded-xl shadow-xl flex flex-col z-10 overflow-hidden mt-8 mb-8 print:bg-white print:border-black print:shadow-none"><button onClick={() => { if(!match.vincitore && activeTorneo.stato !== 'completato' && !isSalaSuspended) richiedePin((sid) => impostaVincitore(turnoIndex, match.id, match.p1, sid), "Vittoria Giocatore 1") }} disabled={!!match.vincitore || isSalaSuspended} className={`p-4 border-b border-gray-800 flex justify-between items-center transition-all print:border-black print:text-black print:bg-white ${match.vincitore?.id === match.p1.id ? 'bg-green-600 text-black font-black print:font-black' : match.vincitore ? 'bg-gray-800 text-gray-500' : 'bg-gray-900 hover:bg-blue-900 text-white font-bold'}`}><span className="uppercase truncate w-full text-left text-sm">{match.p1.nome}</span>{match.vincitore?.id === match.p1.id && <span className="print:text-black">✓</span>}</button>{match.p2 ? (<button onClick={() => { if(!match.vincitore && activeTorneo.stato !== 'completato' && !isSalaSuspended) richiedePin((sid) => impostaVincitore(turnoIndex, match.id, match.p2, sid), "Vittoria Giocatore 2") }} disabled={!!match.vincitore || isSalaSuspended} className={`p-4 flex justify-between items-center transition-all print:text-black print:bg-white ${match.vincitore?.id === match.p2.id ? 'bg-green-600 text-black font-black print:font-black' : match.vincitore ? 'bg-gray-800 text-gray-500' : 'bg-gray-900 hover:bg-blue-900 text-white font-bold'}`}><span className="uppercase truncate w-full text-left text-sm">{match.p2.nome}</span>{match.vincitore?.id === match.p2.id && <span className="print:text-black">✓</span>}</button>) : (<div className="p-4 text-gray-600 font-black text-center uppercase tracking-widest text-xs bg-gray-800/20 print:bg-white print:text-gray-400">BYE</div>)}</div>))}</div>))}</div></div><div className="mt-8 flex gap-4 print:hidden">{activeTorneo.stato === 'in_corso' && !isSalaSuspended && (<button onClick={() => richiedePin((sid) => generaProssimoTurno(sid), "Genera Turno / Concludi")} className="flex-[3] py-6 bg-blue-600 text-white uppercase font-black rounded-3xl hover:bg-blue-500 transition-all shadow-[0_0_20px_rgba(37,99,235,0.5)] active:scale-95 text-xl">AVANZA AL TURNO SUCCESSIVO / CONCLUDI TORNEO</button>)}{isSalaSuspended && activeTorneo.stato === 'in_corso' && (<div className="flex-[3] py-6 bg-red-950/50 border border-red-900 text-red-500 uppercase font-black rounded-3xl text-center text-xl tracking-widest">AVANZAMENTO BLOCCATO (SOLA LETTURA)</div>)}<button onClick={() => { setIsBracketModalOpen(false); setActiveTorneo(null); }} className="flex-[1] py-6 bg-gray-800 text-gray-400 uppercase font-black rounded-3xl hover:bg-gray-700 transition-all text-xl">CHIUDI TABELLONE</button></div></div></div>)}
