...
export default function TorreDiControlloAdmin() {
  ...
  // Il tasto è bloccato se i campi sono vuoti o durante il caricamento
  ...
  const caricaSale = async () => {
    ...
    if (error) {
      ...
      // Filtriamo per mostrare solo record validi, eliminando eventuali "ghost rows"
      const saleValide = data.filter(sala => sala.name && sala.name.trim() !== "");
      setSale(saleValide);
    }
  };
  ...
  const aprireAuditContabile = (sala: any) => {
    let statoPagamenti = sala.scadenza_contributo ? 
        (new Date() > new Date(sala.scadenza_contributo) ? "DA FATTURARE (Scaduto)" : "REGOLARE") : "IN PROVA GRATUITA";
    ...
    <div className="min-h-screen bg-[#050505] p-10 text-white">
      ...
      <div className="bg-[#11131a] p-8 rounded-3xl border border-gray-800 mb-10 max-w-2xl">
        ...
        <button 
          ...
          className={`w-full mt-4 p-4 rounded-xl font-black uppercase ${isFormInvalid ? 'bg-gray-700 cursor-not-allowed text-gray-400' : 'bg-cyan-600 hover:bg-cyan-500'}`}
        >
