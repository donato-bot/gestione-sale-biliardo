...
// OBIETTIVO: App Web per i Giocatori (Design Fedele "Zen", senza distrazioni)
// ==========================================
"use client";
...
export default function SocioPage() {
  ...
  // Stati per la prenotazione ultra-semplificata
  ...
  if (view === "login") return (
    <div className="min-h-screen bg-[#05070a] sm:p-8 md:p-12 flex justify-center items-center">
      <div className="w-full max-w-md bg-black sm:border-2 border-gray-800 sm:rounded-[3rem] sm:shadow-[0_0_60px_rgba(34,197,94,0.15)] min-h-screen sm:min-h-[800px] flex flex-col items-center justify-center p-8 relative overflow-hidden">
        <div className="w-full text-center">
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="La tua Email" className="w-full bg-[#111928] border border-gray-800 p-5 rounded-2xl mb-6 text-center outline-none focus:border-green-500 font-bold text-white shadow-inner" />
  ...
  if (view === "dashboard") return (
    <div className="min-h-screen bg-[#05070a] sm:p-8 md:p-12 flex justify-center items-start">
      <div className="w-full max-w-md bg-black sm:border-2 border-gray-800 sm:rounded-[3rem] sm:shadow-[0_0_60px_rgba(34,197,94,0.15)] min-h-screen sm:min-h-[850px] flex flex-col p-6 relative overflow-hidden">
        ...
        <div className="bg-[#111928] rounded-[2rem] p-8 border border-gray-800 shadow-xl mb-6 relative overflow-hidden">
        ...
        <div className="bg-[#111928] rounded-[2rem] p-6 border border-gray-800 shadow-xl mb-6">
          ...
          {miePrenotazioni.length === 0 ? (
            ...
            <div className="space-y-3">
              {miePrenotazioni.map((p) => {
                 ...
                 return (
                   <div key={p.id} className="bg-black border border-gray-800 p-4 rounded-2xl flex justify-between items-center shadow-md">
                     <div>
  ...
  if (view === "prenota") return (
    <div className="min-h-screen bg-[#05070a] sm:p-8 md:p-12 flex justify-center items-start">
      <div className="w-full max-w-md bg-black sm:border-2 border-gray-800 sm:rounded-[3rem] sm:shadow-[0_0_60px_rgba(34,197,94,0.15)] min-h-screen sm:min-h-[850px] flex flex-col p-6 relative overflow-hidden">
            ...
            <button onClick={() => setView("dashboard")} className="mb-8 self-start text-gray-500 hover:text-white text-[10px] font-black uppercase tracking-widest bg-[#111928] px-4 py-3 rounded-xl border border-gray-800 transition-colors">
              ← Torna alla Plancia
            </button>
            
            <h2 className="text-[2.5rem] leading-none font-black uppercase mb-12 italic text-white drop-shadow-md tracking-tight">
              Scegli sessione
            </h2>
            
            <div className="mb-8">
              <label className="text-[11px] text-green-500 font-black uppercase tracking-widest mb-3 block">1. Data e Ora</label>
              <input 
                type="datetime-local" 
                value={bookingTime} 
                onChange={(e) => setBookingTime(e.target.value)} 
                className="w-full bg-[#111928] text-white font-bold p-5 rounded-2xl outline-none border border-gray-800 focus:border-green-500 cursor-pointer shadow-inner" 
                style={{ colorScheme: 'dark' }}
              />
            </div>

            <div className="mb-8">
              <label className="text-[11px] text-green-500 font-black uppercase tracking-widest mb-3 block">2. Note (Opzionale)</label>
              <textarea 
                value={noteCliente} 
                onChange={(e) => setNoteCliente(e.target.value)} 
                placeholder="Es. Arrivo con 10 min di ritardo, siamo in 4..."
                rows={3}
                className="w-full bg-[#111928] text-white font-bold p-5 rounded-2xl outline-none border border-gray-800 focus:border-green-500 shadow-inner resize-none text-sm placeholder-gray-600"
              />
            </div>
            ...
            <div className="mt-auto pt-2 pb-4">
              <button 
                ...
                className="w-full bg-green-600 hover:bg-green-500 disabled:bg-[#111928] disabled:border-gray-800 disabled:text-gray-600 border border-green-400 py-6 rounded-2xl font-black text-black uppercase tracking-widest transition-all shadow-[0_0_30px_rgba(34,197,94,0.15)] disabled:shadow-none"
              >
