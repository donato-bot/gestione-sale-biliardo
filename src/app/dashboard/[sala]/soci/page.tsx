    ...
    <div className="min-h-screen bg-[#050505] text-white p-4 sm:p-8 font-sans">
      <div className="w-full max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="bg-[#11131a] border border-gray-800/80 rounded-xl overflow-hidden shadow-2xl">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm font-bold text-white divide-y divide-gray-800/40">
              {loading ? (
                ...
                <tr><td colSpan={5} className="p-8 text-center text-gray-600 font-black uppercase tracking-widest text-[10px]">Nessun socio registrato. Inizia aggiungendone uno.</td></tr>
              ) : (
                soci.map((socio) => {
                  const scaduta = isScaduta(socio.scadenza_tessera);
                  return (
                    <tr key={socio.id} className="hover:bg-gray-800/30 transition-colors group">
                      <td className="p-4">
                        <p className="text-base font-black uppercase text-gray-200">{socio.cognome} {socio.nome}</p>
                      </td>
                      <td className="p-4">
                        <p className="text-[10px] text-gray-400 font-bold">{socio.telefono || "—"}</p>
                        <p className="text-[10px] text-gray-500">{socio.email || "—"}</p>
                      </td>
                      <td className="p-4 text-[10px] text-gray-400 font-mono uppercase">
          ...
          <div className="bg-black/80 border-t border-gray-800 p-4 flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-gray-500">
            <span>Totale Tesserati: {soci.length}</span>
            <span className="text-emerald-400">Attivi: {soci.filter(s => !isScaduta(s.scadenza_tessera)).length}</span>
          </div>
        </div>
      </div>
            ...
            <div className="p-8">
               ...
               <form onSubmit={salvaSocio} className="space-y-4">
                 <div className="grid grid-cols-2 gap-4">
                   <div>
                     <label className="block text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1.5">Nome *</label>
                     <input type="text" required value={nome} onChange={(e) => setNome(e.target.value)} className="w-full bg-black border border-gray-800 p-3 rounded-xl text-white font-bold text-sm uppercase focus:outline-none focus:border-cyan-500" />
                   </div>
                   <div>
                     <label className="block text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1.5">Cognome *</label>
                     <input type="text" required value={cognome} onChange={(e) => setCognome(e.target.value)} className="w-full bg-black border border-gray-800 p-3 rounded-xl text-white font-bold text-sm uppercase focus:outline-none focus:border-cyan-500" />
                   </div>
                 </div>

                 <div className="grid grid-cols-2 gap-4">
                   <div>
                     <label className="block text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1.5">Telefono</label>
                     <input type="tel" value={telefono} onChange={(e) => setTelefono(e.target.value)} className="w-full bg-black border border-gray-800 p-3 rounded-xl text-white font-bold text-sm focus:outline-none focus:border-cyan-500" />
                   </div>
                   <div>
                     <label className="block text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1.5">Email</label>
                     <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-black border border-gray-800 p-3 rounded-xl text-white font-bold text-sm focus:outline-none focus:border-cyan-500" />
                   </div>
                 </div>

                 <div>
                   <label className="block text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1.5">Codice Fiscale</label>
                   <input type="text" value={codiceFiscale} onChange={(e) => setCodiceFiscale(e.target.value)} className="w-full bg-black border border-gray-800 p-3 rounded-xl text-white font-bold text-sm uppercase focus:outline-none focus:border-cyan-500" />
                 </div>

                 <div>
                   <label className="block text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1.5">Scadenza Tessera</label>
                   <input type="date" value={scadenzaTessera} onChange={(e) => setScadenzaTessera(e.target.value)} className="w-full bg-black border border-gray-800 p-3 rounded-xl text-white font-bold text-sm focus:outline-none focus:border-cyan-500 custom-calendar-icon" />
                 </div>
