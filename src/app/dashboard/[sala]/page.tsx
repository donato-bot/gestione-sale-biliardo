...
export default function DashboardSala() {
  ...
  const richiedePin = (callback: (staffId: string) => void, descrizione: string) => {
    if (listaStaff.length === 0) { 
        alert("⚠️ Crea prima uno Staff nel menù Staff."); 
        setActiveView("staff");
        return; 
    }
  ...
  // --- TUTTE LE FUNZIONI DI SALVATAGGIO ---

  const salvaNuovoStaff = async () => {
    if (!newStaffNome || newStaffPin.length !== 4) { alert("Dati incompleti!"); return; }
    const { error } = await supabase.from('staff').insert([{ sala_id: currentSalaId, nome: newStaffNome, pin: newStaffPin }]);
    if (error) alert(error.message);
    else { alert("✅ Staff Salvato!"); await refreshDati(currentSalaId!); setIsNewStaffModalOpen(false); setNewStaffNome(""); setNewStaffPin(""); }
  };
  ...
  const salvaRicarica = async (staffId: string) => {
    ...
    alert("✅ Ricarica OK!"); await refreshDati(currentSalaId!); setIsRechargeModalOpen(false);
  };
  ...
  const salvaTariffe = async (staffId: string) => {
    ...
    alert("✅ Tariffe Aggiornate!"); setActiveView("hub");
  };
  ...
  const annullaTransazione = async (id: string, staffId: string) => {
    if (confirm("Stornare operazione?")) { await supabase.from('sessioni').delete().eq('id', id); await refreshDati(currentSalaId!); }
  };
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-green-500 font-black text-2xl tracking-widest italic animate-pulse">SISTEMA IN CARICAMENTO...</div>;
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 font-sans tracking-tighter overflow-x-hidden">
      ...
      {activeView !== "hub" && (<button onClick={() => setActiveView("hub")} className="w-full max-w-6xl mx-auto bg-gray-900 border border-gray-700 text-white py-6 rounded-[2rem] mb-8 font-black uppercase flex items-center justify-center gap-4 transition-all">🔙 MENU PRINCIPALE</button>)}
      ...
      {activeView === 'magazzino' && (
        <div className="max-w-6xl mx-auto animate-in slide-in-from-bottom-8">
          <button onClick={() => setIsNewProductModalOpen(true)} className="w-full mb-8 py-8 bg-blue-600 rounded-[2.5rem] text-white font-black text-2xl uppercase shadow-xl">+ AGGIUNGI PRODOTTO</button>
                ...
                {soci.map((s) => (
                  <tr key={s.id} className="hover:bg-gray-800/30 transition-all">
                    ...
                    <td className="p-6 text-right">
                      <button onClick={() => { setSocioToRecharge(s); setIsRechargeModalOpen(true); }} className="bg-green-600 text-black px-6 py-3 rounded-2xl font-black uppercase shadow-lg">💰 Ricarica</button>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody className="divide-y divide-gray-800">
                {recenti.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-800/20 transition-all">
                    ...
                    <td className="p-5 text-right font-black text-white text-lg">€ {parseFloat(r.costo_totale).toFixed(2)}</td>
                    <td className="p-5 text-center"><button onClick={() => richiedePin((sid) => annullaTransazione(r.id, sid), "Storno Operazione")} className="text-red-500">❌</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeView === 'impostazioni' && (
        <div className="max-w-2xl mx-auto bg-gray-900 p-10 rounded-[3rem] border-4 border-gray-800 animate-in slide-in-from-bottom-8 shadow-2xl">
          <h3 className="text-3xl font-black text-white uppercase italic mb-8 border-b border-gray-800 pb-4">Gestione Tariffe</h3>
          <div className="space-y-8 mb-12">
            <div><label className="block text-gray-500 font-black text-xs uppercase mb-4 tracking-widest">Tariffa Standard (€/h)</label><input type="number" value={tariffaStandard} onChange={(e) => setTariffaStandard(parseFloat(e.target.value))} className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-4xl text-white font-black" /></div>
            <div><label className="block text-yellow-500 font-black text-xs uppercase mb-4 tracking-widest">Tariffa Soci (€/h)</label><input type="number" value={tariffaSoci} onChange={(e) => setTariffaSoci(parseFloat(e.target.value))} className="w-full bg-black border border-yellow-900 p-6 rounded-2xl text-4xl text-white font-black" /></div>
          </div>
