...
export default function DashboardSala() {
  ...
  const richiedePin = (callback: (staffId: string) => void, descrizione: string) => {
    if (listaStaff.length === 0) { alert("⚠️ Registra prima uno Staff dal Menu."); return; }
  ...
  // Funzioni di supporto (uguali a prima)
  const logout = async () => { await supabase.auth.signOut(); router.push('/login'); };
  const apriStatistiche = async () => { setActiveView("statistiche"); setStatsStorico(null); if(!currentSalaId) return; try { const trentaGiorniFa = new Date(); trentaGiorniFa.setDate(trentaGiorniFa.getDate() - 30); const { data: sDB } = await supabase.from('sessioni').select('*').eq('sala_id', currentSalaId).eq('stato', 'terminata').gte('fine', trentaGiorniFa.toISOString()); if(!sDB) return; const incassoMese = sDB.reduce((acc, s) => s.metodo_pagamento === 'credito' ? acc : acc + parseFloat(s.costo_totale || 0), 0); setStatsStorico({ giorni: [], topProdotto: "", topTavolo: "", incassoMese }); } catch(e){} };
    ...
    <div className="min-h-screen bg-black text-white p-4 font-sans tracking-tighter print:hidden">
      ...
      {activeView === 'plancia' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {tavoli.map((t) => (
            <div key={t.id} className={`p-8 rounded-[2.5rem] border-4 transition-colors ${t.stato === 'LIBERO' ? 'border-green-900 bg-gray-950' : 'border-red-600 bg-gray-900'}`}>
              ...
              {t.stato === 'LIBERO' ? (
                <button onClick={() => { setActiveTableId(t.id); setIsStartModalOpen(true); }} className="w-full py-8 bg-green-700 rounded-3xl font-black uppercase text-xl tracking-widest shadow-xl">AVVIA PARTITA</button>
              ) : (
                <div className="flex gap-4">
                  ...
                  }} className="flex-[2] py-8 bg-red-700 rounded-3xl font-black uppercase text-xl tracking-widest shadow-xl">CHIUDI CONTO</button>
                </div>
              )}
            </div>
          ))}
      ...
      {/* MODALI NUOVI (STAFF / START / BAR / SUMMARY) */}
      {isNewStaffModalOpen && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50"><div className="bg-gray-900 border-4 border-cyan-600 p-10 rounded-[3rem] w-full max-w-lg"><h3 className="text-3xl font-black text-cyan-500 mb-8 uppercase italic text-center">Nuovo Dipendente</h3><input value={newStaffNome} onChange={(e) => setNewStaffNome(e.target.value)} placeholder="Nome" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-xl text-white mb-4" /><input type="password" maxLength={4} value={newStaffPin} onChange={(e) => setNewStaffPin(e.target.value)} placeholder="PIN (4 Cifre)" className="w-full bg-black border border-gray-800 p-6 rounded-2xl text-3xl font-mono text-cyan-400 tracking-[0.5em] mb-8" /><button onClick={salvaNuovoStaff} className="w-full py-8 bg-cyan-600 text-black rounded-3xl font-black uppercase text-xl">SALVA</button></div></div>)}
      {isStartModalOpen && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50"><div className="bg-gray-950 border-4 border-blue-600 p-10 rounded-[3rem] w-full max-w-lg"><h3 className="text-3xl font-black text-blue-500 mb-8 uppercase italic text-center">Avvia Tavolo</h3><select value={selectedSocioId} onChange={(e) => setSelectedSocioId(e.target.value)} className="w-full bg-gray-900 border-4 border-blue-900 p-6 rounded-2xl text-xl text-white mb-8"><option value="">👤 Cliente Occasionale</option>{soci.map(s => (<option key={s.id} value={s.id}>🏆 Socio: {s.cognome} {s.nome}</option>))}</select><button onClick={() => richiedePin((staffId) => avviaSessione(staffId), "Avvio Partita")} className="w-full py-8 bg-blue-600 rounded-3xl font-black uppercase text-xl shadow-xl">CONFERMA CON PIN</button></div></div>)}
      {isBarModalOpen && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50"><div className="bg-gray-950 border-4 border-orange-500 p-10 rounded-[3rem] w-full max-w-lg"><h3 className="text-3xl font-black text-orange-500 mb-8 uppercase italic text-center">Aggiungi Bar</h3><select value={selectedProdottoId} onChange={(e) => setSelectedProdottoId(e.target.value)} className="w-full bg-gray-900 p-8 rounded-2xl border border-gray-800 text-xl text-white mb-8"><option value="">Seleziona Prodotto...</option>{prodotti.map(p => (<option key={p.id} value={p.id}>{p.nome} (€{p.prezzo_vendita})</option>))}</select><button onClick={() => richiedePin((staffId) => aggiungiBar(staffId), "Consumazione Bar")} className="w-full py-8 bg-orange-600 rounded-3xl font-black uppercase text-xl shadow-xl">CONFERMA CON PIN</button></div></div>)}
      {isSummaryModalOpen && summaryData && (<div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50"><div className="bg-gray-950 border-4 border-green-600 p-8 rounded-[3rem] w-full max-w-lg shadow-2xl"><h3 className="text-4xl font-black text-green-500 uppercase italic mb-8">Chiusura Conto</h3><div className="bg-gray-900 p-6 rounded-3xl border border-gray-800 mb-8"><div className="flex justify-between text-xl uppercase mb-2"><span>Biliardo</span><span>€ {summaryData.costoBiliardo.toFixed(2)}</span></div><div className="flex justify-between text-xl uppercase text-orange-400 mb-4"><span>Bar</span><span>€ {summaryData.costoBar.toFixed(2)}</span></div><div className="border-t-2 border-gray-700 pt-6 flex justify-between items-center"><span className="text-3xl font-black italic">TOTALE</span><span className="text-5xl font-black text-green-500">€ {summaryData.totale.toFixed(2)}</span></div></div><div className="flex flex-col gap-4"><button onClick={() => richiedePin((staffId) => confermaChiusura('contanti', staffId), "Pagamento Contanti")} className="w-full py-6 bg-green-600 rounded-3xl font-black uppercase text-xl">💵 CONTANTI</button><button onClick={() => richiedePin((staffId) => confermaChiusura('pos', staffId), "Pagamento POS")} className="w-full py-6 bg-blue-600 rounded-3xl font-black uppercase text-xl">💳 POS</button></div></div></div>)}
