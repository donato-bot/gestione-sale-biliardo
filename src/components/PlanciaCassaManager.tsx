...
export default function PlanciaCassaManager({ salaId }: PlanciaCassaProps) {
  ...
  // MOTORE DI CHIUSURA TURNO
  const chiudiTurno = async () => {
    if (movimenti.length === 0) {
      alert("Nessun movimento da chiudere nel turno attuale.");
      return;
    }

    if (!window.confirm("⚠️ ATTENZIONE: Sei sicuro di voler chiudere il turno di cassa?\nTutti i movimenti verranno sigillati nell'Archivio Storico e il cassetto ripartirà da zero.")) {
      return;
    }

    setLoading(true);
    try {
      // Generazione di un ID Turno univoco basato su Data e Ora
      const timestamp = new Date().toISOString().replace(/[-:.]/g, '').slice(0, 14);
      const idTurno = `TURNO-${timestamp}`;

      const { error } = await supabase
        .from("libro_mastro")
        .update({ id_chiusura: idTurno })
        .eq("sala_id", salaId)
        .is("id_chiusura", null); // Sigilla solo i movimenti attualmente aperti

      if (error) throw error;

      alert(`✅ Turno chiuso con successo!\nCodice Archivio: ${idTurno}`);
      await caricaMovimenti(); // Ricarica la cassa (che ora sarà vuota)

    } catch (err: any) {
      alert("Errore durante la chiusura del turno: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* COLONNA DESTRA: LISTA MOVIMENTI E TASTO CHIUSURA */}
          ...
          <div className="bg-[#11131a] border border-gray-800 p-6 rounded-2xl h-full flex flex-col">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <button 
                ...
                className="bg-red-950/40 hover:bg-red-900 border border-red-500/30 text-white disabled:opacity-50 disabled:cursor-not-allowed text-[9px] px-4 py-2 rounded-lg font-black uppercase tracking-widest transition-all shadow-[0_0_10px_rgba(239,68,68,0.2)]"
              >
                🔒 Chiudi Turno di Cassa
              </button>
            </div>
